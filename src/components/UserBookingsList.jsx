...
const UserBookingsList = () => {
  ...
  const { isConnected } = useSSE({
    ...
    onBookingApproved: (data) => {
      console.log('SSE: Booking approved event received');
      setNotification(data.message);
      fetchUserBookings(); // Refresh the list
      setTimeout(() => setNotification(''), 5000);
    },
    onBookingRejected: (data) => {
      console.log('SSE: Booking rejected event received');
      setNotification(data.message);
      fetchUserBookings(); // Refresh the list
      setTimeout(() => setNotification(''), 5000);
    },
    onBookingCompleted: (data) => {
      console.log('SSE: Booking completed event received');
      setNotification(data.message);
      fetchUserBookings(); // Refresh the list
      setTimeout(() => setNotification(''), 5000);
    },
    onBookingCancelled: (data) => {
      console.log('SSE: Booking cancelled event received');
      setNotification(data.message);
      fetchUserBookings(); // Refresh the list
      setTimeout(() => setNotification(''), 5000);
    },
    onBookingUpdated: (data) => {
      console.log('SSE: Booking updated event received');
      setNotification(data.message);
      fetchUserBookings(); // Refresh the list
      setTimeout(() => setNotification(''), 5000);
    }
    ...
    <div className="space-y-4">
      ...
      <div className="bg-blue-50 border border-blue-200 rounded-lg p-4 mb-4">
        ...
        <div className="text-sm text-blue-700 space-y-1">
          ...
          <div><strong>SSE:</strong> {isConnected ? 'Connected' : 'Disconnected'}</div>
          <div><strong>Bookings:</strong> {bookings.length} found</div>
