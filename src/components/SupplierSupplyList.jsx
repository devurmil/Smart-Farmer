...
import { Loader2, AlertCircle, Edit, Trash2, Package, Grid, LayoutList } from 'lucide-react';
    ...
    <div className="space-y-4">
            ...
            <div key={supply.id} className="border rounded-lg p-4 hover:shadow-md transition-shadow">
              <div className="flex items-start justify-between">
                ...
                <div className="flex items-center gap-2 ml-4">
                  ...
                  <Button variant="outline" size="sm" onClick={() => handleDelete(supply.id)} className="text-red-600 hover:text-red-700">
              ...
              {supply.imageUrl && (
                <div className="mt-3">
                  <img src={supply.imageUrl} alt={supply.name} className="w-24 h-24 object-cover rounded border" onError={(e) => { e.target.style.display = 'none'; }} />
                </div>
              )}
            </div>
          ))}
      ...
      {viewMode === 'grid' && (
        <div className="grid grid-cols-1 md:grid-cols-2 lg:grid-cols-3 gap-4">
          {supplies.map((supply) => (
            <div key={supply.id} className="border rounded-lg p-4 hover:shadow-md transition-shadow bg-white">
              {supply.imageUrl && (
                <div className="mb-3">
                  <img src={supply.imageUrl} alt={supply.name} className="w-full h-40 object-cover rounded" onError={(e) => { e.target.style.display = 'none'; }} />
                </div>
              )}
              <h3 className="text-lg font-semibold mb-2">{supply.name}</h3>
              <div className="flex flex-wrap gap-2 mb-2">
                <Badge className={getCategoryColor(supply.category)}>{getCategoryIcon(supply.category)} {supply.category}</Badge>
                <Badge className={getStatusColor(supply.available)}>{supply.available ? 'Available' : 'Unavailable'}</Badge>
              </div>
              <div className="text-sm text-gray-600 mb-2">
                <p><span className="font-medium">Price:</span> ₹{supply.price} / {supply.unit}</p>
                <p><span className="font-medium">Qty:</span> {supply.quantity} {supply.unit}</p>
                {supply.brand && <p><span className="font-medium">Brand:</span> {supply.brand}</p>
                }
                {supply.expiryDate && <p className="text-orange-600"><span className="font-medium">Expires:</span> {new Date(supply.expiryDate).toLocaleDateString()}</p>}
                {supply.description && <p className="text-gray-700 mt-2 line-clamp-2">{supply.description}</p>}
              </div>
